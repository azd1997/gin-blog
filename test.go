...
func main() {
	...
	msg, _ := ParseAnyStructPtrIntoMsg(user, "  ", 3)
	RspWithMsg(msg)
}
...
func parsePairRecursive(val reflect.Value, pairs *[]*Pair, depth int, maxdepth int, filter func(filedName, regExp string)) {
	...
	for i:= 0;i<fieldNum;i++ {
		...
		// TODO: 增加过滤函数实现
		filter(val.Type().Field(i).Name, regExp)
		...
		if valI.Kind() == reflect.Struct && depth < maxdepth {
			*pairs = append(*pairs, &Pair{
			...
			parsePairRecursive(valI, pairs, depth + 1, maxdepth)
			continue
					...
					*pairs = append(*pairs, &Pair{
					...
					for i:=0; i<valI.Len();i++ {
						...
						*pairs = append(*pairs, &Pair{
						...
						parsePairRecursive(valI.Index(i), pairs, depth+2, maxdepth)
					}
					continue
				}
...
func ParseAnyStructPtrIntoMsg(structPtr interface{}, indent string, maxDepth int) (*Msg, error) {
	msg := &Msg{
		Indent: indent,
		MaxDepth: maxDepth,
	}
	...
	parsePairRecursive(val, pairs, 0, maxDepth)
